...
map<unsigned int, __int128_t> MCM_GreedySearch_MCM0(map<__int128_t, unsigned int> Kset, unsigned int N, unsigned int r, map<unsigned int, __int128_t> MCM_0)
{
    // Create initial independent model:
    __int128_t Op = 1;
    ...
    for (unsigned int i = 0; i < r; i++)
    {
        ...
        Best_MCM_LogE_Part[i] = LogE_SubCM(Kset, MCM_0[i], N);
    }
